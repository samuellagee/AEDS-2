...
void quicksort(Restaurante* v[], int esq, int dir, long* comp, long* mov){
	int i = esq, j = dir;
	...
	if(esq < j) quicksort(v, esq, j, comp, mov);
	if(i < dir) quicksort(v, i, dir, comp, mov);
}
...
int main(){
	...
	quicksort(selecionados, 0, qtd - 1, &comp, &mov);

	clock_t fim = clock();

	double tempo = (double)(fim - inicio) / CLOCKS_PER_SEC;

	FILE* log = fopen("845833_quicksort.txt", "w");
