...
void counting_sort(Restaurante* v[], int n, long* comp, long* mov){
	int max = v[0]->capacidade;
	for(int i = 1; i < n; i++){
		if(v[i]->capacidade > max){
			max = v[i]->capacidade;
		}
	}
	...
	for(int i = n - 1; i >= 0; i--){
		output[count[v[i]->capacidade] - 1] = v[i];
		count[v[i]->capacidade]--;
		(*mov)++;
	}
...
int main(){
	...
	counting_sort(selecionados, qtd, &comp, &mov);

	clock_t fim = clock();

	double tempo = (double)(fim - inicio) / CLOCKS_PER_SEC;

	FILE* log = fopen("845833_countingsort.txt", "w");
